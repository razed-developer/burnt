...
fn resolve_tool(app: &tauri::AppHandle, name: &str) -> Result<PathBuf, String> {
    let candidates = tool_candidates(app, name);
    if let Some(found) = candidates.iter().find(|path| path.is_file()) { return Ok(found.clone()); }
    let checked = candidates.iter().map(|path| path.display().to_string()).collect::<Vec<_>>().join("\n- ");
    Err(format!("{name} was not found. Put the portable executable in tools/bin. Checked:\n- {checked}"))
}

fn parse_duration(stdout: &[u8]) -> Result<f64, String> {
    let text = String::from_utf8_lossy(stdout);
    let value = text.lines().map(str::trim).find(|line| !line.is_empty()).ok_or_else(|| "ffprobe did not return a duration".to_string())?;
    let duration = value.parse::<f64>().map_err(|_| format!("Could not parse ffprobe duration: {value}"))?;
    if duration.is_finite() && duration >= 0.0 { Ok(duration) } else { Err("ffprobe returned an invalid duration".to_string()) }
}
...
pub fn probe_audio(app: &tauri::AppHandle, source_path: &str) -> Result<AudioMetadata, String> {
    ...
    Ok(AudioMetadata { duration_seconds: parse_duration(&output.stdout)? })
}

fn create_session_dir() -> Result<PathBuf, String> {
    let stamp = SystemTime::now().duration_since(UNIX_EPOCH).map_err(|error| format!("System clock error: {error}"))?.as_millis();
    let dir = std::env::temp_dir().join("Burnt").join(format!("session-{stamp}"));
    fs::create_dir_all(&dir).map_err(|error| format!("Could not create temporary audio folder: {error}"))?;
    Ok(dir)
}

fn pad_to_cd_sector(path: &Path) -> Result<(u64, u64), String> {
    let len = fs::metadata(path).map_err(|error| format!("Could not inspect prepared PCM: {error}"))?.len();
    let remainder = len % CD_SECTOR_BYTES;
    if remainder != 0 {
        let padding = (CD_SECTOR_BYTES - remainder) as usize;
        let mut file = OpenOptions::new().append(true).open(path).map_err(|error| format!("Could not pad prepared PCM: {error}"))?;
        file.write_all(&vec![0u8; padding]).map_err(|error| format!("Could not pad prepared PCM: {error}"))?;
    }
    let bytes = fs::metadata(path).map_err(|error| format!("Could not verify prepared PCM: {error}"))?.len();
    Ok((bytes, bytes / CD_SECTOR_BYTES))
}

pub fn prepare_audio(app: &tauri::AppHandle, source_paths: &[String]) -> Result<Vec<PreparedTrack>, String> {
    if source_paths.is_empty() { return Ok(Vec::new()); }
    ...
    let result = (|| {
        ...
        for (index, source_path) in source_paths.iter().enumerate() {
            ...
            let (bytes, sectors) = pad_to_cd_sector(&pcm_path)?;
            prepared.push(PreparedTrack { source_path: source_path.clone(), pcm_path: pcm_path.display().to_string(), duration_seconds: metadata.duration_seconds, bytes, sectors });
        }
        Ok(prepared)
    })();
    if result.is_err() { let _ = fs::remove_dir_all(&session_dir); }
    result
}

pub fn cleanup_prepared_tracks(pcm_paths: &[String]) -> Result<(), String> {
    for pcm_path in pcm_paths {
        ...
        if let Some(parent) = path.parent() {
            if parent.file_name().and_then(|name| name.to_str()).is_some_and(|name| name.starts_with("session-")) { let _ = fs::remove_dir(parent); }
        }
    }
    Ok(())
}
